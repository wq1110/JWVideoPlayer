...
public class StorageManager {
    private static Logger logger = LoggerFactory.getLogger(StorageManager.class.getName());
    ...
    private static class CacheFileInfo {
        public String mFilePath;//缓存文件路径
        public long mLastModified;//缓存文件最后修改日期
        public long mSize;//缓存文件大小
    ...
    private void initCacheInfoInternal() {
        ...
        try {
            ...
            logger.w("cleanExpiredCacheData failed, exception = " + e.getMessage());
        }
    ...
    private void checkCacheInternal(String filePath) {
        ...
        try {
            ...
            logger.w( "setLastModifiedTimeStamp failed, exception="+e.getMessage());
        }
