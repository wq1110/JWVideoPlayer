package com.media.jwvideoplayer.cache.m3u8;
...
import com.media.jwvideoplayer.cache.utils.UrlUtils;
...
/**
 * Created by Joyce.wang on 2023/4/2.
 * M3U8的通用处理类
 */
public class M3U8Utils {
    private static Logger logger = LoggerFactory.getLogger(M3U8Utils.class.getName());
    ...
    public static M3U8 parseNetworkM3U8Info(String parentUrl, String videoUrl, Map<String, String> headers, int retryCount) throws IOException {
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (line.startsWith(VideoCacheConstants.TAG_PREFIX)) {
                    if (line.startsWith(VideoCacheConstants.TAG_MEDIA_DURATION)) {
                        String ret = parseStringAttr(line, VideoCacheConstants.REGEX_MEDIA_DURATION);
                        if (!TextUtils.isEmpty(ret)) {
                            segDuration = Float.parseFloat(ret);
                        }
                    } else if (line.startsWith(VideoCacheConstants.TAG_TARGET_DURATION)) {
                        String ret = parseStringAttr(line, VideoCacheConstants.REGEX_TARGET_DURATION);
                        if (!TextUtils.isEmpty(ret)) {
                            targetDuration = Integer.parseInt(ret);
                        }
                    } else if (line.startsWith(VideoCacheConstants.TAG_VERSION)) {
                        String ret = parseStringAttr(line, VideoCacheConstants.REGEX_VERSION);
                        if (!TextUtils.isEmpty(ret)) {
                            version = Integer.parseInt(ret);
                        }
                    } else if (line.startsWith(VideoCacheConstants.TAG_MEDIA_SEQUENCE)) {
                        String ret = parseStringAttr(line, VideoCacheConstants.REGEX_MEDIA_SEQUENCE);
                        if (!TextUtils.isEmpty(ret)) {
                            sequence = Integer.parseInt(ret);
                        }
                    } else if (line.startsWith(VideoCacheConstants.TAG_STREAM_INF)) {
                        hasMasterList = true;
                    } else if (line.startsWith(VideoCacheConstants.TAG_DISCONTINUITY)) {
                        hasDiscontinuity = true;
                    } else if (line.startsWith(VideoCacheConstants.TAG_ENDLIST)) {
                        hasEndList = true;
                    } else if (line.startsWith(VideoCacheConstants.TAG_KEY)) {
                        hasKey = true;
                        method = parseOptionalStringAttr(line, VideoCacheConstants.REGEX_METHOD);
                        String keyFormat = parseOptionalStringAttr(line, VideoCacheConstants.REGEX_KEYFORMAT);
                        if (!VideoCacheConstants.METHOD_NONE.equals(method)) {
                            keyIv = parseOptionalStringAttr(line, VideoCacheConstants.REGEX_IV);
                            if (VideoCacheConstants.KEYFORMAT_IDENTITY.equals(keyFormat) || keyFormat == null) {
                                if (VideoCacheConstants.METHOD_AES_128.equals(method)) {
                                    // The segment is fully encrypted using an identity key.
                                    String tempKeyUri = parseStringAttr(line, VideoCacheConstants.REGEX_URI);
                                    if (tempKeyUri != null) {
                    ...
                    } else if (line.startsWith(VideoCacheConstants.TAG_INIT_SEGMENT)) {
                        String tempInitSegmentUri = parseStringAttr(line, VideoCacheConstants.REGEX_URI);
                        if (!TextUtils.isEmpty(tempInitSegmentUri)) {
                            hasInitSegment = true;
                            initSegmentUri = UrlUtils.getM3U8MasterUrl(videoUrl, tempInitSegmentUri);
                            segmentByteRange = parseOptionalStringAttr(line, VideoCacheConstants.REGEX_ATTR_BYTERANGE);
                        }
                    }
                    continue;
                }
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (line.startsWith(VideoCacheConstants.TAG_PREFIX)) {
                    if (line.startsWith(VideoCacheConstants.TAG_MEDIA_DURATION)) {
                        ...
                        initSegmentUri = parseStringAttr(line, VideoCacheConstants.REGEX_URI);
    ...
    public static void createLocalM3U8File(File m3u8File, M3U8 m3u8) throws Exception{
        ...
        try {
            ...
            bfw.write(VideoCacheConstants.PLAYLIST_HEADER + "\n");
            bfw.write(VideoCacheConstants.TAG_VERSION + ":" + m3u8.getVersion() + "\n");
            bfw.write(VideoCacheConstants.TAG_MEDIA_SEQUENCE + ":" + m3u8.getSequence() + "\n");
            bfw.write(VideoCacheConstants.TAG_TARGET_DURATION + ":" + m3u8.getTargetDuration() + "\n");
            for (M3U8Seg m3u8Ts : m3u8.getSegList()) {
                if (m3u8Ts.hasInitSegment()) {
                    ...
                    bfw.write(VideoCacheConstants.TAG_INIT_SEGMENT + ":" + initSegmentInfo + "\n");
                }
                if (m3u8Ts.isHasKey() && !TextUtils.isEmpty(m3u8Ts.getMethod())) {
                    ...
                    bfw.write(VideoCacheConstants.TAG_KEY + ":" + key + "\n");
                }
                if (m3u8Ts.isHasDiscontinuity()) {
                    bfw.write(VideoCacheConstants.TAG_DISCONTINUITY + "\n");
                }
                bfw.write(VideoCacheConstants.TAG_MEDIA_DURATION + ":" + m3u8Ts.getDuration() + ",\n");
                bfw.write(m3u8Ts.getUrl());
                bfw.newLine();
            }
            bfw.write(VideoCacheConstants.TAG_ENDLIST);
            bfw.flush();
        } catch (Exception e){
            logger.w("createLocalM3U8File failed exception = " + e.getMessage());
            if (m3u8File.exists()) {
                m3u8File.delete();
            }
            throw e;
    ...
    public static boolean updateM3U8TsPortInfo(File proxyM3U8File, int proxyPort) {
        ...
        if (tempM3U8File != null) {
            ...
            try {
                ...
                logger.w( "Create buffered writer file failed, exception="+e);
                ProxyCacheUtils.close(bfw);
                return false;
            }
            ...
            try {
                ...
                logger.w("Create stream reader failed, exception="+e);
                ProxyCacheUtils.close(inputStreamReader);
                return false;
            }
            ...
            if (inputStreamReader != null && bfw != null) {
                ...
                try {
                    ...
                    logger.w("Read proxy m3u8 file failed, exception="+e);
                    return false;
                } finally {
                    ProxyCacheUtils.close(bfw);
                    ProxyCacheUtils.close(inputStreamReader);
