...
public class M3U8SegResponse extends BaseResponse {
    private static Logger logger = LoggerFactory.getLogger(M3U8SegResponse.class.getName());
    ...
    public M3U8SegResponse(HttpRequest request, String sourceId, String parentUrl, String videoUrl, Map<String, String> headers, long time, String fileName) throws Exception {
        super(request, videoUrl, headers, time);
        mSourceId = sourceId;
        mParentUrl = parentUrl;
        mSegUrl = videoUrl;
        mSegFile = new File(mCachePath, fileName);
        logger.i( "SegFilePath="+mSegFile.getAbsolutePath());
        ...
        logger.i("index="+mSegIndex+", parentUrl="+mParentUrl+", segUrl="+mSegUrl);
        VideoProxyCacheManager.getInstance().seekToCacheTaskFromServerForM3u8(mSourceId, mParentUrl, mSegIndex);
    }
    ...
    public void sendBody(Socket socket, OutputStream outputStream, long pending) throws Exception {
        if (mFileName.startsWith(ProxyCacheUtils.INIT_SEGMENT_PREFIX)) {
            ...
            logger.d("FileLength=" + mSegFile.length() + ", segLength=" + mSegLength + ", FilePath=" + mSegFile.getAbsolutePath());
        }
        ...
        try {
            ...
            while(shouldSendResponse(socket, mM3U8Md5)) {
                ...
                logger.d( "Send M3U8 ts file end, this="+this);
                break;
            }
        } catch (Exception e) {
            throw e;
